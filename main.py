...
def ejecutar_pipeline_con_batches(tamaño_batch=1000):
    ...
    # [WARNING] LIMITAR CPU A 2 CORES FÍSICOS (throttle a nivel de proceso)
    proceso = psutil.Process()
    try:
        ...
        logging.info(f"[CONFIG] Limitando proceso a {cores_a_usar} cores físicos: {cores_asignados}")
    except Exception as e:
        logging.warning(f"[WARNING] No se pudo aplicar cpu_affinity: {e}")
    ...
    try:
        ...
        logging.info(f"[LOAD] Ingesta con procesamiento en batches de {tamaño_batch} filas...")
        ...
        if df_raw is not None:
            ...
            logging.info(f"[STATS] Total de filas: {total_filas} | Batches: {num_batches} x {tamaño_batch}")
            ...
            for i in range(num_batches):
                ...
                logging.info(f"[PROCESS] Procesando batch {i+1}/{num_batches} (filas {inicio}-{fin})...")
                ...
                if batch_limpio.empty:
                    logging.warning(f"  [WARNING] Batch {i+1} quedó vacío después de limpieza")
                    continue
                ...
                if batch_transformado.empty:
                    logging.warning(f"  [WARNING] Batch {i+1} quedó vacío después de transformación")
                    continue
                ...
                if batch_validado.empty:
                    logging.warning(f"  [WARNING] Batch {i+1} quedó vacío después de validación")
                    continue
                ...
                logging.info(f"  [OK] Batch {i+1} procesado | CPU: {cpu_actual:.1f}% | RAM: {ram_actual:.1f} MB")
                
                # [PAUSE] THROTTLE: Pausar 0.3s entre batches para evitar picos de CPU
                if i < num_batches - 1:  # No pausar después del último batch
                    time.sleep(0.3)
            
            # Combinar todos los batches procesados
            logging.info("[COMBINE] Combinando batches procesados...")
            df_transformado = pd.concat(dfs_procesados, ignore_index=True)
            logging.info(f"[PACKAGE] Dataset final: {len(df_transformado)} filas")
            
            # [PAUSE] THROTTLE: Pausar antes de exportación para dejar que CPU descanse
            logging.info("[PAUSE] Preparando exportación (throttle CPU)...")
            time.sleep(0.5)
            
            # Paso 5: Exportar el resultado final
            logging.info("[SAVE] Exportando datos procesados...")
            def exportar_local():
                try:
                    # Intentar Parquet primero (columnar, 3-5x más eficiente)
                    logging.info("[PACKAGE] Intentando formato Parquet...")
                    df_transformado.to_parquet('datos_preprocesados.parquet', compression='snappy', index=False)
                    logging.info("[OK] Exportado a Parquet exitosamente (columnar comprimido)")
                except Exception as e:
                    # Fallback a CSV si Parquet falla
                    logging.warning(f"[WARNING] Parquet falló ({type(e).__name__}), usando CSV...")
                    try:
                        df_transformado.to_csv('datos_preprocesados.csv', index=False, encoding='utf-8')
                        logging.info("[OK] Exportado a CSV exitosamente")
                    except Exception as e2:
                        logging.error(f"[ERROR] Error en exportación CSV: {e2}", exc_info=True)
            medir_etapa("5.1 Exportación Local", exportar_local)
            
            # [PAUSE] THROTTLE: Pausar después de exportación
            logging.info("[PAUSE] Descansando después de exportación...")
            time.sleep(1.0)
            
            # Paso 6: Carga a Supabase
            logging.info("[UPLOAD] Cargando a Supabase...")
            exito = medir_etapa("5.2 Carga a Supabase", cargar_datos_supabase, df_transformado, "transacciones_ml_procesadas", URL_SUPABASE)
            logging.info("[OK] Dataset final exportado (formato optimizado)")
            ...
            logging.error("[ERROR] El pipeline se detuvo en la capa de ingesta. Verifica el archivo.")
            
    except ValueError as ve:
        logging.critical(f"[ERROR] FALLO CRÍTICO DE CALIDAD DE DATOS: {ve}")
    except Exception as e:
        logging.critical(f"[ERROR] Fallo inesperado en el orquestador principal: {e}", exc_info=True)
...
def ejecutar_pipeline():
    ...
    try:
        ...
        if df_raw is not None:
            ...
            def exportar_local():
                try:
                    logging.info("[PACKAGE] Probando formato Parquet...")
                    df_transformado.to_parquet('datos_preprocesados.parquet', engine='pyarrow', compression='snappy', index=False)
                    logging.info("[OK] Exportación a Parquet exitosa (columnar comprimido)")
                except Exception as e:
                    logging.warning(f"[WARNING] Parquet no disponible ({e}), usando CSV...")
                    df_transformado.to_csv('datos_preprocesados.csv', index=False)
                    logging.info("[OK] Exportación a CSV exitosa")
            medir_etapa("5.1 Exportación Local", exportar_local)
